...
- **Aşama 68 (TAMAMLANDI ✅):** Akıllı Anahtar Havuzu (Key Pool) Cooldown & Anlık 429 Kota Rotasyonu Mimarisi (`src/services/geminiService.js`). Yeniden analiz ("Try AI Analysis Again") butonuna basıldığında veya boru hattı çalıştığında; bir anahtarın kotası dolduğunda (`429 Rate Limit / Quota exceeded / RESOURCE_EXHAUSTED`), o anahtar anında 60 saniyelik bekleme moduna (`keyCooldownMap`) alınarak aynı istek düşürülmeden havuzdaki bir sonraki aktif anahtara aktarılır. Sonraki tüm istekler otomatik olarak kısıtlanmamış geçerli anahtardan başlatılır. 24/24 Jest entegrasyon testi ile doğrulandı.
